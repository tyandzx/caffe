...
## Image IO
def load_image_cv2(filename, color=True):
    """
    Load an image by opencv
    Give
    image: opencv.imread()
    """
...
def resize_image_cv2(im, new_dims, interp = cv2.INTER_LINEAR):
    """
    Resize an image array with cv2.resize
    """
    resized_im = cv2.resize(im, new_dims, interpolation=interp)
    resized_im_uint8 = resized_im.astype(np.uint8)
    return resized_im_uint8.astype(np.float32)
